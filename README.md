...
- 同一无人机相邻投放间隔 $\ge 1\,\mathrm{s}$，每架至多 3 枚。

| 导弹 | 初始位置 | \(t_{\mathrm{hit}}\) (s) |
|------|----------|--------------------------|
...
- FY1：$v=120$，朝向假目标，$t_{\mathrm{drop}}=1.5\,\mathrm{s}$，$\tau=3.6\,\mathrm{s}$
- 圆柱严格全遮蔽：**1.3621 s**（点模型中心约 1.4055 s，圆柱更严）
...
| 航向 | 179.6475° |
| 速度 | 139.9983 m/s |
| 弹1 | \(t_d=0.0030\), \(\tau=3.6111\) |
| 弹2 | \(t_d=3.7025\), \(\tau=5.3375\) |
| 弹3 | \(t_d=5.5695\), \(\tau=6.0405\) |
| 并集 | **7.608993 s** |
...
| FY2 | M2 | 293.6618 | 140.0 | 2 | 双弹并集 ~7.75 s |
...
**公平吞吐基准**（同一批 $N=800$ 样本、$n_t=3000$ 时间点，仅换目标点数）：
